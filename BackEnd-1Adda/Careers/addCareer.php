    ...
    <div id="main">
        ...
        $("#fileToUpload").change(function(){
          ...
          $('#formData').on("submit",function(e){
            ...
            if(submit==1){
                                                  //Propagate only if urls are valid
              var form=$('#formData');
              ...
              $.ajax({                                            //request a unique id for the mentor
                ...
                data:{careerRequest:1},
                async:false,
                success(id)
                {
                  careerUniqueId="CRE_"+id;
              ...
              img.append('c',careerUniqueId);                           //Therefore mentor Id is also sent
              $.ajax({
                type: 'POST',
                processData: false,
                contentType: false,
              ...
              if(dontAdd==0){                               //If any profiles were added
               ...
               $.ajax({                                     //data is sent to enter into the database
                  ...
                  url:'addCareerToDb.php',
                  data:{formdata:data,i:imgPath,c:careerUniqueId},
                  async:false,
                  success:function(data){
                    alert("Record was succesfully inserted with Id:"+careerUniqueId);
