...
<!-- In comments , replace mentor with advisor , explanation is as same as that for Mentors -->
          ...
          $('#formData').on("submit",function(e){
            ...
            if(submit==1){                                        //Propagate only if urls are valid
              ...
              $.ajax({                                            //request a unique id for the mentor
                ...
                data:{partnerRequest:1},
                async:false,
                success(id)
                {
                  partnerUniqueId="PRT_"+id;
              ...
              img.append('p',partnerUniqueId);                           //Therefore mentor Id is also sent
              $.ajax({
                type: 'POST',
                processData: false,
                contentType: false,
              ...
              if(dontAdd==0){                               //If any profiles were added
               ...
               $.ajax({                                     //data is sent to enter into the database
                  ...
                  url:'addPartnerToDb.php',
                  data:{formdata:data,i:imgPath,p:partnerUniqueId},
                  async:false,
                  success:function(data){
                    alert("Record was succesfully inserted with Id:"+partnerUniqueId);
